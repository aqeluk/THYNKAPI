...
from src.tasks.schemas import Task, Task_Pydantic, TaskIn_Pydantic
...
async def update_task(task_id: int, task: TaskIn_Pydantic, current_user=Depends(get_current_user)):
    updated_task = await Task.filter(id=task_id, user_id=current_user.id).update(**task.dict())
    if updated_task:
