...
async def get_user_businesses(current_user=Depends(get_current_user)):
    try:
        response = await user_business_pydantic.from_queryset(UserBusiness.filter(owner=current_user))
        return {"status": "ok", "data": response}
    except DoesNotExist as dne:
        raise BusinessNotFoundException(str(dne))
    except Exception as e:
        raise ServerErrorException(str(e))


@router.get('/get/{business_id}')
async def get_specific_user_business(business_id: int, current_user=Depends(get_current_user)):
    if business_id == 0:
...
async def delete_user_business(business_id: int, current_user=Depends(get_current_user)):
    if business_id == 0:
        raise InvalidIdException("Invalid business id")
    business = await UserBusiness.get(id=business_id)
    owner = await User.filter(id=business.owner_id).first()
    if current_user.username not in ["root", "master"] or current_user.id != owner.id:
...
async def add_user_product(business_id: int, product_details: user_product_pydanticIn, current_user=Depends(get_current_user)):
    try:
        business = await UserBusiness.get(id=business_id)
...
async def get_all_business_products(business_id: int, current_user=Depends(get_current_user)):
    if business_id == 0:
...
async def get_specific_user_product(product_id: int, current_user=Depends(get_current_user)):
    try:
...
async def update_user_product(product_id: int, update_info: user_product_pydanticIn, current_user=Depends(get_current_user)):
    try:
        product = await UserProduct.get(id=product_id)
        business = await product.business
        if current_user.id != business.owner_id:
...
async def delete_user_product(product_id: int, current_user=Depends(get_current_user)):
    if product_id == 0:
        raise InvalidIdException("Invalid product id")
    try:
        product = await UserProduct.get(id=product_id)
        ...
        # Get the base path of your project
...
async def create_upload_file(product_id: int, file: UploadFile = File(...), current_user=Depends(get_current_user)):
    if product_id == 0:
        raise InvalidIdException("Invalid product id")
    try:
        product = await UserProduct.get(id=product_id)
